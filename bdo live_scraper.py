...
if not search_query.strip():
    ...
    if not st.session_state.stored_places:
        with st.spinner(f"Searching all configured Uganda directories & registries for '{search_query}' in {region}..."):
            all_records = []
            ...
            for source in DIRECTORY_SOURCES:
                ...
                try:
                    ...
                    # Defensive validation: a source may not return records that
                    # actually match the current query/region. Never rewrite an
                    # old record to make it appear to match.
                    records = [
                        r for r in records
                        if query_match(r, search_query, region)
                    ]
                    st.session_state.source_status[source_name] = len(records)
                    all_records.extend(records)
                except Exception as exc:
                    st.session_state.source_status[source_name] = 0
                    st.session_state.source_errors[source_name] = str(exc)[:200]

            df_temp = normalize_and_dedupe(all_records)
            if not df_temp.empty:
                # Store only records belonging to THIS fingerprint.
    ...
    if st.session_state.stored_places:
        ...
        # Final guard against stale/cross-search records.
        if "Search Fingerprint" in st.session_state.stored_places[0]:
        ...
        st.success("✅ Search completed. Results belong only to the current region + business keyword. No records are fabricated or renamed from a previous search.")
